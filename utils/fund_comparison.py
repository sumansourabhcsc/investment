...
# Drop this file into utils/fund_comparison.py
# Then call show_fund_comparison() from Tools.py
#
# What it does:
#   • Select 2–3 funds (portfolio or custom code)
#   • Fetches NAV history from mfapi.in for each
#   • Normalises all NAVs to 100 on the same start date
#   • Overlaid NAV growth chart (rebased)
#   • Side-by-side metrics: CAGR, Max Drawdown, Volatility (std dev of daily returns),
#     Sharpe proxy, Best/Worst month
# ══════════════════════════════════════════════
...
# ─────────────────────────────────────────────
# Shared helpers (mirrors Tools.py style)
# ─────────────────────────────────────────────
...
def _chart_layout(height: int = 420) -> dict:
    return dict(
        ...
        margin=dict(t=28, b=28, l=12, r=12),
        legend=dict(
...
# ─────────────────────────────────────────────
# NAV fetcher (same pattern as Tools.py)
# ─────────────────────────────────────────────
...
def _fetch_nav_series(fund_code: str) -> dict:
    ...
    Returns {"name": str, "navs": pd.Series(date→float)} or {"error": str}.
    """
    try:
        r = requests.get(
...
def _calc_metrics(series: pd.Series, label: str) -> dict:
    """
    Given a NAV series (date-indexed, sorted), compute:
      CAGR, Max Drawdown, Annualised Volatility, Sharpe proxy,
    ...
    ann_vol   = daily_ret.std() * np.sqrt(252) * 100   # % p.a.

    # ── Sharpe proxy (CAGR / volatility, risk-free ≈ 6%) ──
...
# Palette for up to 3 funds
_COLORS = ["#00f5d4", "#bf80ff", "#ffb347"]

def _metric_row(label: str, values: list[str], colors: list[str], bold_best_idx: int | None = None):
    """Render one metric row across N fund columns."""
    cells = ""
    ...
    st.markdown(
        ...
        f'color:rgba(255,255,255,0.45);">{label}</td>'
        f'{cells}</tr>',
        unsafe_allow_html=True,
    )


def _render_metrics_table(metrics_list: list[dict], colors: list[str]):
    """Full side-by-side metrics table for 2–3 funds."""
    ...
    def _fmt(v, suffix="", decimals=2, prefix=""):
        return f"{prefix}{v:.{decimals}f}{suffix}" if v is not None else "—"

    def _best_idx(vals: list, higher_is_better=True):
        """Return index of best value; None if all are None."""
    ...
    _metric_row(
        f"Abs Return ({metrics_list[0]['years']:.1f}y)",
        [_fmt(v, "%") for v in abs_vals],
        colors,
        _best_idx(abs_vals),
    )

    # ── Max Drawdown (higher = less negative = better) ──
    dd_vals = [m["max_dd"] for m in metrics_list]
    _metric_row(
        "Max Drawdown",
        [_fmt(v, "%") for v in dd_vals],
        colors,
        _best_idx(dd_vals, higher_is_better=True),   # least negative wins
    )

    # ── Annualised Volatility (lower is better) ──
    vol_vals = [m["ann_vol"] for m in metrics_list]
    _metric_row(
        "Volatility (ann.)",
        [_fmt(v, "%") for v in vol_vals],
        colors,
        _best_idx(vol_vals, higher_is_better=False),
    )
    ...
    _metric_row(
        "Sharpe (proxy)",
        [_fmt(v, "", decimals=2) for v in sharpe_vals],
        colors,
        _best_idx(sharpe_vals),
    )
    ...
    _metric_row(
        "Worst Month",
        [_fmt(v, "%") for v in wm_vals],
        colors,
        _best_idx(wm_vals, higher_is_better=True),
    )
...
# ─────────────────────────────────────────────
# Main entry point — call this from Tools.py
# ─────────────────────────────────────────────
...
def show_fund_comparison():
    ...
    .winner-badge {
        ...
        font-size: 12px; font-weight: 700; color: #00f5d4;
    ...
    with left:
        ...
        fund_selections = []   # list of {"label", "code"}
        ...
        for i in range(num_funds):
            ...
            if source == "My Portfolio":
                # Exclude already-chosen portfolio funds
    ...
    with right:
        if not compare_btn:
            st.markdown(
                ...
                'Select funds on the left and hit Compare</div>'
                '<div style="font-size:12px;margin-top:6px;opacity:0.7;">'
                'CAGR · Max Drawdown · Volatility · Sharpe · Best/Worst Month'
                '</div></div>',
                unsafe_allow_html=True,
            )
            return
        ...
        # ── Fetch NAV data ──
        raw_data = {}   # code → {"name", "navs": Series}
        fetch_errors = []
        ...
        for idx, sel in enumerate(fund_selections):
            ...
            if "error" in result:
                fetch_errors.append(f"Fund {sel['label']} (code {sel['code']}): {result['error']}")
            else:
                # Override display label with API name, shortened
                raw_data[sel["code"]] = {
                    "name":  result["name"],
                    "label": sel["label"],   # user's chosen short label
                    "navs":  result["navs"],
                }
        progress.empty()
        ...
        # ── Align to common start date (latest of all fund starts) ──
        ...
        # ══════════════════════════════════════════
        # CHART 1 — Rebased NAV Growth (base = 100)
        # ══════════════════════════════════════════
        ...
        metrics_list = []
        for code in codes:
            m = _calc_metrics(clipped[code]["navs"], clipped[code]["label"])
        ...
        def _winner_line(emoji, label, idx, metric_str):
            ...
            return (
                ...
                f'<span style="color:rgba(255,255,255,0.35);font-size:12px;">({metric_str})</span>'
                f'</div>'
            )
        ...
        summary_html += _winner_line(
            "🏆", "Best CAGR", best_cagr_idx,
            f'{metrics_list[best_cagr_idx]["cagr"]:.2f}% p.a.'
        )
        summary_html += _winner_line(
            "🛡️", "Least Drawdown", best_dd_idx,
            f'{metrics_list[best_dd_idx]["max_dd"]:.2f}%'
        )
        summary_html += _winner_line(
            "📐", "Lowest Volatility", best_vol_idx,
            f'{metrics_list[best_vol_idx]["ann_vol"]:.2f}% ann.'
        )
        if best_sharpe_idx is not None:
            summary_html += _winner_line(
                "⚖️", "Best Risk-Adjusted (Sharpe)", best_sharpe_idx,
                f'{sharpe_vals[best_sharpe_idx]:.2f}'
            )
        summary_html += "</div>"
        ...
        st.markdown(
            ...
            '⚠️ <strong>Disclaimer:</strong> Metrics are computed from historical NAV data '
            'fetched via mfapi.in. Past performance is not indicative of future returns. '
            'Sharpe ratio uses 6% as the assumed risk-free rate.'
            '</div>',
            unsafe_allow_html=True,
        )
